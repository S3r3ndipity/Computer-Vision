function varargout = guiMAC(varargin)
% GUIMAC MATLAB code for guiMAC.fig
%      GUIMAC, by itself, creates a new GUIMAC or raises the existing
%      singleton*.
%
%      H = GUIMAC returns the handle to a new GUIMAC or the handle to
%      the existing singleton*.
%
%      GUIMAC('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUIMAC.M with the given input arguments.
%
%      GUIMAC('Property','Value',...) creates a new GUIMAC or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUIMAC before guiMAC_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to guiMAC_OpeningFcn via varargin.
%
%      *See GUIMAC Options on GUIDE's Tools menu.  Choose "GUIMAC allows only one
%      instance to run (singleton)".
%
% See also: GUIDE, GUIDATA, GUIHANDLES

% Edit the above text to modify the response to help guiMAC

% Last Modified by GUIDE v2.5 28-Dec-2014 18:37:14
...
% --- Executes just before guiMAC is made visible.
function gui_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to guiMAC (see VARARGIN)

% Choose default command line output for guiMAC
...
% UIWAIT makes guiMAC wait for user response (see UIRESUME)
